...
class MultiAgentRummyEnv(ParallelEnv):
    ...
    # Define Gym-compatible spaces
        self.observation_space = spaces.Box(low=0, high=1, shape=(114,), dtype=np.float32)  # Correct shape
...
# Observation and action spaces
    ...
    def observe(self, agent):
        """
        Return the detailed observation for the given agent.
        """
        hand = self.hands[agent]
        obs = np.zeros(54, dtype=np.float32)  # Encode player's hand
    ...
    # Metadata: Deck ratio and round ratio
        max_deck_size = 54
        deck_ratio = len(self.deck) / max_deck_size
        round_ratio = self.rounds / self.max_rounds

    # Discard pile history (last 3 cards)
        discard_pile_obs = np.zeros(54, dtype=np.float32)
        for card in self.discard_pile[-3:]:
            discard_pile_obs[self._card_to_index(card)] += 1


    # Combine all components into a single observation
        obs = np.concatenate([obs, [deck_ratio, round_ratio], discard_pile_obs]).astype(np.float32)

    # Validate the observation
        if obs.shape != self.observation_space.shape:
           raise ValueError(f"Generated observation {obs} does not match the expected shape {self.observation_space.shape}")
        if not self.observation_space.contains(obs):
           raise ValueError(f"Generated observation {obs} has values outside the valid range")
    ...
    def step(self, action):
        """
        Perform the action for the current agent and move to the next agent.
        """
        rewards = {}
        dones = {}
    ...
    # Discard Action
        ...
        # Reward for blocking opponents
           if discarded_card in self._predict_opponent_strategy():
              reward += 5
              self._track_metrics({"blocks": 1})
           else:
            # Penalize for helping opponents
              reward -= 5
              self._track_metrics({"help_opponent": 1})
    ...
    # Evaluate the hand for rewards
        reward += self._evaluate_hand(hand)
    ...
    # Increment Rounds
        ...
        # End the game if the maximum rounds are reached
           self.dones = {a: True for a in self.agents}
    ...
    # Return observation, reward, done status, and additional info
        ...
        def observe(self, agent):
            """
            Return a detailed observation for the given agent.
            """
            hand = self.hands[agent]
    ...
    # Metadata: Deck ratio, round ratio
        deck_ratio = len(self.deck) / 54
        round_ratio = self.rounds / self.max_rounds
    ...
    # Opponent last action
        opponent_last_action = np.zeros(54, dtype=np.float32)
        if len(self.discard_pile) > 0:
           opponent_last_action[self._card_to_index(self.discard_pile[-1])] = 1

    # Concatenate observations
        obs = np.concatenate([obs, [deck_ratio, round_ratio], discard_pile_obs, opponent_last_action])
        return obs
    ...
    def _track_metrics(self, metrics):
        """
        Update and log strategic performance metrics.
        """
        self.metrics["blocks"] += metrics.get("blocks", 0)
        self.metrics["high_point_penalty"] += metrics.get("high_point_penalty", 0)
        self.metrics["help_opponent"] += metrics.get("help_opponent", 0)



def make_rummy_env(max_rounds=100):
    env = RummyEnv(RummyEnv(max_rounds=100, num_players=2))
    obs, _ = env.reset()
    print("Observation shape:", obs.shape)  # Should print (114,)
...
def train_multiagent_model(env, total_timesteps=5000, num_envs=2):
    """
...
if __name__ == "__main__":
    ...
    env = MultiAgentRummyEnv()
    model = train_multiagent_model(env, total_timesteps=50000)
